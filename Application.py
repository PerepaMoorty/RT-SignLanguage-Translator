...
while not exit:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            if (
                button_position_x1 < mouse_x < button_position_x1 + button_width
                and button_position_y1 < mouse_y < button_position_y1 + button_height
            ):
                ...
                Train_And_Save()
                screen = pygame.display.set_mode(
                    (screen_width, screen_height), flags=pygame.SHOWN
                )
            elif (
                button_position_x2 < mouse_x < button_position_x2 + button_width
                and button_position_y2 < mouse_y < button_position_y2 + button_height
            ):
    ...
    screen.blit(
        text_title,
        (screen_width / 2 - text_title.get_size()[0] / 2, (screen_height / 5)),
    )

    # Start Program Button
    color_button_1 = (
        COLOR_BUTTON_HOVER
        if button_position_x1 < mouse_x < button_position_x1 + button_width
        and button_position_y1 < mouse_y < button_position_y1 + button_height
        else COLOR_BUTTON_IDLE
    )
    pygame.draw.rect(
        screen,
        color_button_1,
        (button_position_x1, button_position_y1, button_width, button_height),
    )
    screen.blit(
        button_text1,
        (
            button_position_x1 + (button_width - button_text1.get_width()) / 2,
            button_position_y1 + (button_height - button_text1.get_height()) / 2,
        ),
    )
    ...
    color_button_2 = (
        COLOR_BUTTON_HOVER
        if button_position_x2 < mouse_x < button_position_x2 + button_width
        and button_position_y2 < mouse_y < button_position_y2 + button_height
        else COLOR_BUTTON_IDLE
    )
    pygame.draw.rect(
        screen,
        color_button_2,
        (button_position_x2, button_position_y2, button_width, button_height),
    )
    screen.blit(
        button_text2,
        (
            button_position_x2 + (button_width - button_text2.get_width()) / 2,
            button_position_y2 + (button_height - button_text2.get_height()) / 2,
        ),
    )
